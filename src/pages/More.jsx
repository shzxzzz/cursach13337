...
const CoursePage = () => {
    ...
    const { course, loading, error: courseError, refresh } = useCourse(id);
    ...
    useEffect(() => {
        const checkEnrollment = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:3000/api/enrollments?student_id=${user.id}&course_id=${id}`, {
                    headers: {
                ...
                if (response.ok) {
                    ...
                    const enrollment = enrollments.find(e =>
                        e.course_id === parseInt(id) && e.student_id === user.id
                    );
                    setIsEnrolled(!!enrollment);
                }
            } catch (err) {
                console.error('Ошибка проверки записи на курс:', err);
                setEnrollmentError('Не удалось проверить статус записи на курс');
            } finally {
                setIsCheckingEnrollment(false);
            }
        };
    ...
    const handleEnroll = async () => {
        ...
        if (isEnrolled) {
            ...
            navigate('/cabinet/?message=already-enrolled&course=' + course.id);
            return;
        }
        ...
        try {
            ...
            navigate('/cabinet/?enrolled=true&course=' + course.id);

        } catch (error) {
            console.error('Ошибка записи на курс:', error);
            setEnrollmentError(error.message || 'Произошла ошибка при записи на курс');
            setErrorModal({
    ...
    if (loading || isCheckingEnrollment) {
        return (
            <div style={{
                display: 'flex',
        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    <p style={{ color: '#627084', marginBottom: '25px' }}>{courseError}</p>
                    <button
                ...
                <div ref={containerRef} style={{ maxWidth: '1300px', margin: '0 auto', position: 'relative' }}>
                    ...
                    <div style={{ marginBottom: '50px' }}>
                        ...
                        <button
                            ...
                            style={{
                                padding: '16px 45px',
                                backgroundColor: isEnrolled ? '#4caf50' : '#1a79ff',
                                border: 'none',
                                color: '#fff',
                                borderRadius: '10px',
